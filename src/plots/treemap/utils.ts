import { isArray, findIndex } from '@antv/util';
...
export function enableDrillInteraction(options: TreemapOptions): boolean {
  const { interactions, enableDrillDown } = options;
  return enableDrillDown || enableInteraction(interactions, 'treemap-drill-down');
}
...
export function getFommatInteractions(options: TreemapOptions): TreemapOptions['interactions'] {
  ...
  const { hierarchyConfig, drillDownConfig } = options;
  ...
  if (enableDrillDown) {
    ...
    const interaction = deepAssign({}, interactions[index], {
      ...
      cfg: { hierarchyConfig, drillDownConfig },
    });
